...
function recruitmentEvaluation(state,empireId,territoryId,amount=STRATEGIC_RECRUITMENT.baseAmount){
  ...
  else if(safeStrategicInteger(faction.gold)<cost.gold)reason=`Necesitas ${cost.gold} oro.`;
  else if(safeStrategicInteger(faction.food)<cost.food)reason=`Necesitas ${cost.food} comida.`;
