# AGENTS.md — Manual Maestro del Proyecto "Imperios del Mundo"

> **Instrucción para Codex:** Este archivo es tu fuente de verdad. Léelo completo antes de tocar código. Trabaja SOLO la fase activa indicada en la sección 6. No avances de fase sin aprobación explícita de Gabriel.

---

## 1. Contexto del proyecto

**Imperios del Mundo** es un juego de gran estrategia en HTML/JS/CSS puro (un solo archivo, sin frameworks ni backend), creado por Gabriel Catalán con Codex como planner. Combina:
...
- Codex (chat) = Planner + Revisor. Diseña mecánicas, balance y fases.
- **Codex (tú) = Ejecutor.** Implementas exactamente lo que dice este manual, fase por fase.
...
1. `git init` + `.gitignore` + commit inicial con `imperios-del-mundo-5.html` y este `AGENTS.md`.
...
4. **Guardado automático con localStorage** (la PWA sí lo permite, a diferencia del artifact de Codex.ai): autoguardar `saveGame()` al inicio de cada ronda y el legado al terminar partidas, con try/catch y manteniendo los códigos manuales como respaldo/exportación.
...
### 📋 FASE 2 — Balance y dificultad (backlog, requiere diseño previo con Codex chat)
...
- Al terminar cada fase, genera un resumen corto para que Gabriel lo pegue en el chat de Codex (planner) para revisión: qué se hizo, commits, resultado de pruebas, y dudas.
